...
> = C["responsiveVariants"] extends string[]
	? {
			[Screen in WithInitialScreen<
				C["responsiveVariants"]
			>[number]]?: StringToBoolean<keyof V[K]>;
		}
	: {
		...
		? {
				[K in keyof EV]?: isTrueOrArray<C["responsiveVariants"]> extends true
					?
							...
							| TVScreenPropsValue<EV, ES, K, C>
							| undefined
					: StringToBoolean<keyof EV[K]> | undefined;
			} & ClassProp<ClassValue>
		: {
				[K in keyof V | keyof EV]?: isTrueOrArray<
					C["responsiveVariants"]
				> extends true
					?
							| (K extends keyof V ? StringToBoolean<keyof V[K]> : never)
