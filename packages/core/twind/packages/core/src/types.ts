...
export type TypedAtRules = {
	[key in TypedAtRulesKeys]?: key extends `@layer ${string}`
		? MaybeArray<CSSBase>
		: CSSBase;
};
...
export type CSSFontFace = CSS.AtRule.FontFaceFallback &
	CSS.AtRule.FontFaceHyphenFallback;

// eslint-disable-next-line @typescript-eslint/no-empty-interface
export interface CSSNested
	extends Record<
		string,
		CSSProperties | MaybeArray<CSSObject | string> | Falsey
	> {}
...
export type HashFunction = (
	value: string,
	defaultHash: (value: string) => string,
) => string;
...
export type ExtractThemes<
	Theme,
	Presets extends Preset<any>[],
> = UnionToIntersection<
...
export interface TwindUserConfig<
	Theme = BaseTheme,
...
> {
	...
	preflight?:
		| false
		| MaybeArray<
				Preflight | PreflightThunk<BaseTheme & ExtractThemes<Theme, Presets>>
		  >;
...
export type ThemeSectionResolver<Value, Theme extends BaseTheme = BaseTheme> = (
...
export type ThemeConfig<Theme extends BaseTheme = BaseTheme> =
	PartialTheme<Theme> & {
...
export type PresetThunk<Theme = BaseTheme> = (
	config: TwindConfig<Theme & BaseTheme>,
) => TwindPresetConfig<Theme>;

export type Preset<Theme = BaseTheme> =
	| TwindPresetConfig<Theme>
	| PresetThunk<Theme>;
