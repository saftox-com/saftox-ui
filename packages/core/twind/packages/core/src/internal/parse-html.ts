...
export function parseHTML(
  ...
  for (let position = 0; position < markupState.length; position++) {
    const char = markupState[position] as string

    if (mode === MODE_TEXT) {
      if (char === '<') {
        mode = markupState.substr(position + 1, 3) === '!--' ? MODE_COMMENT : MODE_TAGNAME
      }
    } else if (mode === MODE_COMMENT) {
      // Ignore everything until the last three characters are '-', '-' and '>'
      if (char === '>' && markupState.slice(position - 2, position) === '--') {
        mode = MODE_TEXT
      }
    } else if (quote) {
      if (char === quote && markupState[position - 1] !== '\\') {
        commit(position)
      ...
      attributeName = markupState.slice(startIndex, position)
      mode = MODE_ATTRIBUTE
      startIndex = position + 1
    } else if (char === '/' && (mode < MODE_ATTRIBUTE || markupState[position + 1] === '>')) {
      commit(position)
      mode = MODE_SLASH
    } else if (/\s/.test(char)) {
      // <a class=font-bold>
