...
function serialize$<Theme extends BaseTheme = BaseTheme>(
	...
	for (let key in style || {}) {
		...
		if (typeof value === "object" && !Array.isArray(value)) {
			...
			name =
				(value as string) +
				hash(JSON.stringify([precedence, important, style]));
			...
			maxPropertyPrecedence = Math.max(
				...
				declarationPropertyPrecedence(key),
			);
				...
				asArray(value)
					.map((value) =>
						context.s(
							...
							resolveThemeFunction(`${value}`, context.theme) +
								(important ? " !important" : ""),
						),
					)
					.join(";");
