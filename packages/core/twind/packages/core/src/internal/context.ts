	...
	const ctx: Context<Theme> = {
		...
		v(value) {
			if (!variantCache.has(value)) {
				variantCache.set(
					value,
					find(value, variants, variantResolvers, getVariantResolver, ctx) ||
						`&:${value}`,
				);
			}
...
function createResolveFunction<Theme extends BaseTheme = BaseTheme>(
	...
	return createResolve(
		patterns,
		fromMatch(resolve as keyof CSSProperties, convert),
	);
}
...
function createResolve<Result, Theme extends BaseTheme = BaseTheme>(
	...
	return createRegExpExecutor(
		patterns,
		(value, condition, context, isDark?: boolean) => {
