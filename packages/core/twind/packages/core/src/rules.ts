...
> = RuleResolver<Theme, ThemeMatchResult<Value>>;

export type ThemeMatchConverter<
	Value,
	Theme extends BaseTheme = BaseTheme,
> = MatchConverter<Theme, ThemeMatchResult<Value>>;
	...
	return typeof resolve === "function"
		...
		: typeof resolve === "string" && /^[\w-]+$/.test(resolve) // a CSS property alias
			...
			: (match) =>
					...
					resolve ||
					({
						[match[1] as string]: maybeNegate(match, 2),
					} as CSSObject);
}
...
function maybeNegate<T>(
	match: MatchResult,
	offset: number,
	value: T | string = match.slice(offset).find(Boolean) ||
		match.$$ ||
		match.input,
): T | string {
...
>(
	...
	resolve?:
		| keyof CSSProperties
		| ThemeRuleResolver<ThemeValue<Theme[Section]>, Theme>,
...
>(
	...
	const factory: (
		...
		typeof resolve === "string"
			...
			: resolve ||
				(({ 1: $1, _ }, context, section) =>
					({ [$1 || section]: _ }) as CSSObject);

	return withAutocomplete$(
		(match, context) => {
			const themeSection = camelize((section || match[1]) as string) as Section;
...
export type FilterByThemeValue<Theme, Value> = {
	[key in keyof Theme & string]: ThemeValue<Theme[key]> extends Value
		? Theme[key]
		: never;
};
...
export interface ColorFromThemeOptions<
	Theme extends BaseTheme = BaseTheme,
	Section extends keyof FilterByThemeValue<
		Theme,
		ColorValue
	> = keyof FilterByThemeValue<Theme, ColorValue>,
	OpacitySection extends keyof FilterByThemeValue<
		Theme,
		string
	> = keyof FilterByThemeValue<Theme, string>,
> {
...
>(
	...
	return withAutocomplete$(
		(match, context) => {
			...
			const opacityValue =
				(context.theme(opacitySection, opacityMatch || "DEFAULT") as
					| string
					| undefined) ||
			...
			const create =
				resolve ||
				(({ _ }) => {
					const properties = toCSS(property as string, _);
		...
		DEV &&
			((match, context) => {
				const {
					section = `${camelize(match[0]).replace("-", "")}Color` as Section,
					opacitySection = section.replace(
						"Color",
						"Opacity",
					) as OpacitySection,
				} = options;
				...
				const opacities = Object.entries<string>(
					context.theme(opacitySection) || {},
				).filter(
					([key, value]) =>
						key !== "DEFAULT" &&
						/^[\w-]+$/.test(key) &&
						...
						.map(
							([key, value]): AutocompleteItem => ({
								...
								color: toColorValue(value as ColorValue, {
									opacityValue:
										(context.theme(opacitySection, "DEFAULT") as string) || "1",
								}),
								modifiers:
...
export function toCSS(
	property: string,
	value: string | ColorFromThemeValue,
): CSSObject {
			...
			!(
				...
				(
					...
					(/image/i.test(section) &&
						!(/^image:/.test(value) || /^[a-z-]+\(/.test(value))) ||
					// font-*
