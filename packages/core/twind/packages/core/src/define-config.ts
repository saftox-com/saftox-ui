...
>({
	...
	...userConfig
}: TwindUserConfig<Theme, Presets>): TwindConfig<
	BaseTheme & ExtractThemes<Theme, Presets>
> {
	...
	for (const preset of asArray([
		...presets,
		{
			...
			preflight:
				userConfig.preflight !== false && asArray(userConfig.preflight),
			theme: userConfig.theme as TwindConfig<
				BaseTheme & ExtractThemes<Theme, Presets>
			>["theme"],
		...
		const {
			...
			? preset(config)
			: (preset as TwindPresetConfig<Theme>);
