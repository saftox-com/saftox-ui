...
export function parse(token: string): ParsedRule[] {
	...
	if (!parsed) {
			...
			if (startIndex !== position) {
				...
				if (isRule) {
					createRule(
						active,
						current,
						DEV && [startIndex, position + endOffset],
					);
				}
			}
		...
		for (; position < token.length; position++) {
			const char = token[position] as string;
			...
			if (skip) {
				...
				if (
					token[position - 1] !== "\\" &&
					comment.test(token.slice(position))
				) {
					comment = null;
				...
				if (char === ")") {
					// Close nested block
					const nested = active[lastGroup - 1] as string;
						...
						createRule(
							[
								...
								define(
									...
									nested.length > 1
										? nested.slice(0, -1) +
												hash(JSON.stringify([nested, rules]))
										: `${nested}(${format(rules)})`,
									Layer.a,
									rules,
									/@$/.test(nested),
		...
		cache.set(token, (parsed = current[0] as ParsedRule[]));
	}
