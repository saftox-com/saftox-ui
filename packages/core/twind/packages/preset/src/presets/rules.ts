...
// import { __DEV__ } from '@stafna-ui/utils';

const __DEV__ = process.env.NODE_ENV !== "production";

function withAutocomplete$(
	rule: Rule<SaftoxTheme>,
...
const rules: Rule<SaftoxTheme>[] = [
	...
	withAutocomplete("-?(order)-(\\d+)", __DEV__ && (() => range({ end: 12 }))),
	...
	withAutocomplete$(
		...
		__DEV__ && (({ 1: $1 }) => ($1 === "auto" ? [""] : range({ end: 7 }))),
	),
	...
	// Grid Auto Flow
	match("grid-flow-((row|col)(-dense)?)", "gridAutoFlow", (match) =>
		spacify(columnify(match)),
	),
	...
	withAutocomplete$(
		match("(place-(content|items|self))-", ({ 1: $1, $$ }) => ({
			[$1 as "place-content"]:
				("wun".includes($$[3] as string) ? "space-" : "") + $$,
		})),
		__DEV__ &&
			(({ 2: $2 }) =>
				$2 === "content"
				...
				: {
						[`--tw-${
							$3 // diagonal-fractions, stacked-fractions
								? "numeric-fraction"
								: "pt".includes($2[0] as string) // proportional-nums, tabular-nums
									? "numeric-spacing"
									: $2 // lining-nums, oldstyle-nums
										? "numeric-figure"
										: // ordinal, slashed-zero
											$1
						}` as "numeric-spacing"]: $1,
						fontVariantNumeric:
	...
	withAutocomplete$(
		...
		__DEV__ && (() => ["solid", "double", "dotted", "dashed", "wavy"]),
	),
	...
	withAutocomplete$(
		...
		__DEV__ && (() => ["normal", "nowrap", "pre", "pre-line", "pre-wrap"]),
	),
	...
	matchTheme(
		...
		({ 1: $1, _ }) => {
			const corners =
				...
				)[$1 as string] || ([$1, $1] as string[]);

			return {
				[`border-${position(corners[0])}-radius` as "border-top-left-radius"]:
					_,
				[`border-${position(corners[1])}-radius` as "border-top-right-radius"]:
					_,
			};
		},
	),
	...
	matchTheme("divide-([xy])(?:$|-)", "divideWidth", ({ 1: $1, _ }) => {
		...
		return {
			"&>:not([hidden])~:not([hidden])": {
				...
				[`border-${position(edges[0] as string)}Width`]:
					`calc(${_} * calc(1 - var(--tw-divide-${$1}-reverse)))`,
				[`border-${position(edges[1] as string)}Width`]:
					`calc(${_} * var(--tw-divide-${$1}-reverse))`,
			},
		};
	}),
	...
	matchTheme("transition(?:$|-)", "transitionProperty", (match, { theme }) => ({
		...
		transitionTimingFunction:
			match._ === "none"
				? undefined
				: join(theme("transitionTimingFunction", "")),
		transitionDuration:
			match._ === "none" ? undefined : join(theme("transitionDuration", "")),
	})),
	...
	matchTheme("animate(?:$|-)", "animation", (match, { theme, h, e }) => {
		...
		const keyframeValues = theme("keyframes", parts[0]) as unknown as CSSBase;

		if (keyframeValues) {
			return {
				[`@keyframes ${(parts[0] = e(
					h(parts[0] as string),
				))}` as "@keyframes xxx"]: keyframeValues,
	...
	// Transform
	"(transform)-(none)",
	match("transform", transformDefaults),
	...
	/* INTERACTIVITY */
	// Appearance
	withAutocomplete$("(appearance)-", __DEV__ && (() => ["auto", "none"])),

	// Columns
	matchTheme("(columns)-" /*, 'columns' */),
	withAutocomplete$("(columns)-(\\d+)", __DEV__ && (() => range({ end: 12 }))),
	...
	// Margin
	matchTheme<SaftoxTheme, "scrollMargin">(
		"-?scroll-m([xytrbl])?(?:$|-)",
		"scroll-margin",
		edge("scroll-margin"),
	),
	...
	// Pointer Events
	withAutocomplete$("(pointer-events)-", __DEV__ && (() => ["auto", "none"])),

	// Will Change
	matchTheme("(will-change)-" /*, 'willChange' */),
	withAutocomplete$(
		"(will-change)-",
		__DEV__ && (() => ["auto", "contents", "transform"]),
	),
	...
	// Stroke Width
	matchTheme("stroke-", "stroke-width"),
	...
	matchTheme("line-clamp-", "lineClamp", ({ _ }) => {
		return {
			overflow: "hidden",
			display: "-webkit-box",
...
function columnify(value: string | MatchResult): string {
	return (typeof value === "string" ? value : value?.[1] ?? "").replace(
		"col",
		"column",
	);
}
...
function join(
	...
	return (
		value && `${(value as ThemeMatchResult<MaybeArray<string>>)._ || value}`
	);
}
...
function edge(
	...
	propertySuffix = "",
): ThemeRuleResolver<string | ColorFromThemeValue, SaftoxTheme> {
	return ({ 1: $1, _ }) => {
		const edges =
			{
				x: "lrse",
		...
		if (edges) {
			const firstEdgeCSS = toCSS(
				`${propertyPrefix}-${position(edges[0] as string)}${propertySuffix}`,
				_,
			);
			const secondEdgeCSS = toCSS(
				`${propertyPrefix}-${position(edges[1] as string)}${propertySuffix}`,
				_,
			);
		...
		return toCSS(`${propertyPrefix}${propertySuffix}`, _);
	};
}

function filter(prefix = ""): Rule<SaftoxTheme>[] {
	const filters = [
		"blur",
		"brightness",
		"contrast",
	...
	for (const key of filters) {
		defaults[`--tw-${prefix}${key}` as "--tw-blur"] =
			"var(--tw-empty,/*!*/ /*!*/)";
	}
	...
	return [
		...
		...filters.map((key) =>
			matchTheme<SaftoxTheme, "hueRotate" | "dropShadow">(
...
function transformDefaults(): CSSObject {
	return {
		...asDefaults({
			"--tw-translate-x": "0",
			"--tw-translate-y": "0",
...
function range({
	...
	step = 1,
}: { start?: number; end: number; step?: number }): string[] {
	const result: string[] = [];
