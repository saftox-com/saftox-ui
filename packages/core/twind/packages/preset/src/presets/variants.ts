...
const variants: Variant<SaftoxTheme>[] = [
	...
	[
		...
		withAutocomplete$(
			(
				{ 1: $1 /* aria or data */, $$ /* everything after the dash */ },
				context,
			) =>
				$$ &&
				`&[${$1}-${
					...
					[
						...
						.map(
							(key): AutocompleteItem => ({
								suffix: key,
								label: `&[${$1}-${
									theme($1 as string, key) || `${key}="true"`
								}]`,
		...
		withAutocomplete$(
			(
				...
				const selector =
					...
					(($4?.[0] === "["
						? $4
						: (v(($4 as string).slice(1)) as string)) as string);
	...
	[
		...
		withAutocomplete$(
			({ 1: $1 }) => `[dir="${$1}"] &`,
			__DEV__ &&
				(({ 1: $1 }) => [{ prefix: $1, suffix: "", label: `[dir="${$1}"] &` }]),
		),
	],
			...
			__DEV__ &&
				((_, { theme }) =>
					Object.entries(theme("screens") || {})
						...
						.concat([
							{ suffix: "[", label: "@media not all and (min-width: …)" },
						])),
		),
	],
