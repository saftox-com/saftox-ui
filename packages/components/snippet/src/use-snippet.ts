...
export function useSnippet(originalProps: UseSnippetProps) {
  ...
  const [omitVariantProps, variantProps] = mapPropsVariants(originalProps, snippet.variantKeys)
  ...
  const onCopy = () => {
    ...
    if (typeof children() === 'string') {
      ...
      children.toArray().map((child) => {
        const childString = typeof child === 'string' ? child : child?.toString()
    ...
    const valueToCopy = () => local.codeString || stringValue || preRef()?.textContent || ''
