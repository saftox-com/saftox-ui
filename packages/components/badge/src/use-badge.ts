...
export function useBadge(originalProps: UseBadgeProps) {
  ...
  const [omitVariantProps, variantProps] = mapPropsVariants(originalProps, badge.variantKeys)

  const [local, rest] = splitProps(omitVariantProps, ['ref', 'as', 'class', 'classes', 'content'])
