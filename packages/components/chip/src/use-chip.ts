...
export function useChip(originalProps: UseChipProps) {
	...
	const properties = {
		...
		get isOneChar() {
			return (
				typeof children() === "string" && children()?.toString().length === 1
			);
		},
		get hasStartContent() {
	...
	const getContentClone: GetContentCloneProps = (content) => {
		...
		if (typeof content === "function") {
			const isProxyFunction = resolved
				.toString()
				.includes("function () { [native code] }");
	...
	return {
		...
		get startContent() {
			return (
				getAvatarClone(local.avatar) || getContentClone(local.startContent)
			);
		},
		get endContent() {
