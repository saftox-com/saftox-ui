...
const rules: Rule<SaftoxTheme>[] = [
  ...
  withAutocomplete$(
    ...
    __DEV__ && (() => ['start', 'end', 'center', 'between', 'around', 'evenly']),
  ),
  ...
  withAutocomplete$(
    match('(place-(content|items|self))-', ({ 1: $1, $$ }) => ({
      [$1 as 'place-content']: ('wun'.includes($$[3] as string) ? 'space-' : '') + $$,
    })),
    __DEV__ &&
      (({ 2: $2 }) =>
        $2 === 'content'
  ...
  match(
    ...
    'backgroundImage',
    ({ 1: $1 }) => `linear-gradient(to ${position($1 as string, ' ')},var(--tw-gradient-stops))`,
  ),
  ...
  matchTheme('animate(?:$|-)', 'animation', (match, { theme, h, e }) => {
    ...
    if (keyframeValues) {
      return {
        [`@keyframes ${(parts[0] = e(h(parts[0] as string)))}` as '@keyframes xxx']: keyframeValues,
  ...
  withAutocomplete$('(will-change)-', __DEV__ && (() => ['auto', 'contents', 'transform'])),
...
function position(shorthand: string, separator = '-'): string {
  ...
  for (const short of shorthand) {
    longhand.push(
      { t: 'top', r: 'right', b: 'bottom', l: 'left', s: 'start', e: 'end' }[short] as string,
    )
  }
